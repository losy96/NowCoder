...
public class PrintMinNumber {
    /**
     * 方法：重写排序规则，两个字符串进行拼接比较，小的拼接方式则是两个字符串的相对位置排序。
     * 步骤：从数组中选择两个数字，通过下述方法进行这两个数字相对位置的确定。
     *      两个数字前后交换拼接，如：str1+str2 比 str2+str1小，
     *      意味着str1与str2的相对位置一定是str1在str2之前，中间无论是否加入其他数字，都不影响
     *      推广到所有数组中的数字，确定两两数字之间相对位置，即是利用当前确定相对位置的方法进行重写排序规则。
     *      最后将排序好的元素按顺序拼接起来即可。
     * @param numbers
     * @return
     */
